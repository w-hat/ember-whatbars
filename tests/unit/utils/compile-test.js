...
test('it should return a blank document fragment', function(assert) {
  ...
  assert.equal(result.meta.revision, 'WhatBars@0.0.2');
  assert.equal(typeof result.raw, 'object');
  assert.equal(result.raw.statements.length, 0);
  assert.equal(result.raw.locals.length, 0);
  assert.equal(result.raw.templates.length, 0);
...
test('it should return a document fragment with a statement', function(assert) {
  ...
  //assert.equal(result.raw.statements.length, 1);
});
